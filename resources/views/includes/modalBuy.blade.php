<div class="bg-black bg-opacity-50 h-screen w-full hidden drop-shadow-2xl " id="modalBuy">
    <div class="w-[500px] h-[650px] top-[20%] left-[20%] fixed rounded-xl bg-[#FFFFFF]" id="modal">
        ...
        <form action="{{ route('saveBuy', $product->id) }}" class="px-[19px] pt-0 pb-[25px]" method="POST">
            @csrf
            <div class="flex flex-col border-none">
                <x-modalInput title="Ваше имя" name="name"/>
                <x-modalInput title="Телефон" name="number"/>
                <x-modalInput title="Электронная почта" name="email"/>
                <label>
                    <p class="border-solid text-[#333333] font-[Manrope] text-base font-bold pb-[6px]">
                        Текст заказа
                    </p>
                    <textarea class="pl-3 w-full h-32 border rounded border-solid border-[#CCCCCC] resize-none" type="text" name="text"></textarea>
                </label>
                <div class="pt-6">
                    <label>
