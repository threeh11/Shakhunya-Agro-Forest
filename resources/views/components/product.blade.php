<a href="{{ route('product', $products[$i]->id) }}" >
    <div {{ $attributes->class(["w-[286px] h-[375px] flex flex-col justify-between border rounded-[5px] p-[16px] mr-[10px] grow-0 shrink-[0] mb-[10px] sm:mx-0 sm:justify-center sm:mb-[10px] "]) }}>
        
            <img src="{{ asset(Helper::getIMagesForArrayInDb($products[$i]->id)[1]) }}" alt=1>
        
        <div class="flex flex-row justify-between">
            <p class="font-[Manrope] font-normal not-italic text-[14px] text-gray">г. {{ $products[$i]->city }}</p>
        ...
        <p class="font-[Manrope] font-normal not-italic text-[16px] text-black break-keep">{{ $products[$i]->name }}</p>
        <p class="font-[Manrope] font-bold not-italic text-[16px] text-black">{{ $products[$i]->price }}</p>
    </div>
</a>